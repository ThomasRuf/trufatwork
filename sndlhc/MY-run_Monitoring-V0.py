...
if not options.auto:   # default online/offline mode
   for nStart in atlasEvents:
      for n in range(nStart,nStart+800):
         event = M.GetEvent(n)
# trigger on DS or US
         mult = {'scifi': [0]*100,'mufi2': [0]*100,'mufi3': [0]*100}
         for aHit in event.Digi_MuFilterHits:
            if not aHit.isValid(): continue
            detID = aHit.GetDetectorID()
            s    = detID//10000
            if s>1:
              allChannels = M.map2Dict(aHit,'GetAllSignals')
              for c in allChannels:
                  daq = monitorTasks['daq'].R.daqChannel(aHit,c)
                  boardN = int(daq["board"][0].split('_')[1])
                  mult['mufi'+str(s)][boardN] += 1
         if max(mult['mufi'+str(2)])<7:        US = False
         else:                 US = True
         if max(mult['mufi'+str(3)])<3:        DS = False
         else:                 DS = True

         if not US and UStrigger: continue
         if not DS and DStrigger: continue
         for m in monitorTasks:
