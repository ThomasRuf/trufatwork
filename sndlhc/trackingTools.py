...
# do the fit
 ...
 def hitsTokeys(self):
    ...
    for nHit in range(self.event.Digi_ScifiHits.GetEntries()):
             DetID2Key[self.event.Digi_ScifiHits[nHit].GetDetectorID()] = nHit
    for nHit in range(self.event.Digi_MuFilterHits.GetEntries()):
             DetID2Key[self.event.Digi_MuFilterHits[nHit].GetDetectorID()] = nHit
    return DetID2Key

 def timeDirection(self,theTrack):
 # take measurements, calculate T particle
    DetID2Key = 
